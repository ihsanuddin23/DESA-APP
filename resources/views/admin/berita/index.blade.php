...
@section('content')
    ...
    <form method="GET" class="filter-bar">
        <div class="search-wrap">
            ...
            <input type="text" name="search" class="filter-input" placeholder="Cari judul berita..."
                    ...
                    @forelse($berita as $item)
                        <tr>
                            <td class="ps-4">
                                ...
                                @if ($item->ringkasan)
                                    <div style="color:#94a3b8;font-size:.75rem;">{{ Str::limit($item->ringkasan, 60) }}
                                    </div>
                                @endif
                            </td>
                            <td>
                            ...
                            <td>
                                @if ($item->status === 'published')
                                    <span class="status-badge badge-success"><i class="bi bi-circle-fill"
                                            style="font-size:.4rem;"></i>Published</span>
                                @else
                                    <span class="status-badge badge-gray"><i class="bi bi-circle-fill"
                                            style="font-size:.4rem;"></i>Draft</span>
                                @endif
                            </td>
        ...
        @if ($berita->hasPages())
            <div
                ...
                <span style="font-size:.78rem;color:#94a3b8;">Menampilkan
                    {{ $berita->firstItem() }}–{{ $berita->lastItem() }} dari {{ $berita->total() }}</span>
