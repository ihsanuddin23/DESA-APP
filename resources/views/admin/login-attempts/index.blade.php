...
<div class="page-header">
    ...
    <div class="d-flex gap-2">
        @php
            ...
            $failedToday  = \App\Models\LoginAttempt::whereDate('attempted_at', today())->where('successful', false)->count();
        @endphp
        <div style="background:#f0fdf4;border:1px solid #86efac;border-radius:.6rem;padding:.5rem .9rem;text-align:center;">
        ...
        <div style="background:#fef2f2;border:1px solid #fca5a5;border-radius:.6rem;padding:.5rem .9rem;text-align:center;">
...
<form method="GET" class="filter-bar">
    <div class="search-wrap">
        ...
        <input type="text" name="email" class="filter-input" placeholder="Filter email..." value="{{ request('email') }}">
    </div>
    <div class="search-wrap">
        <i class="bi bi-geo-alt si"></i>
        <input type="text" name="ip" class="filter-input" placeholder="Filter IP..." value="{{ request('ip') }}" style="min-width:150px;">
    </div>
    ...
    @if(request()->hasAny(['email','ip','successful']))
        <a href="{{ route('admin.login-attempts.index') }}" style="font-size:.82rem;color:#94a3b8;text-decoration:none;">
...
<div class="data-card">
                ...
                <tr>
                    ...
                    <td>
                        @if($attempt->successful)
                            <span class="status-badge badge-success"><i class="bi bi-check-circle-fill"></i>Berhasil</span>
                        @else
                    ...
                    <td>
                        @if($attempt->failure_reason)
                            @php
                                ...
                                [$rc, $rl] = $reasonMap[$attempt->failure_reason] ?? ['badge-gray', $attempt->failure_reason];
                            @endphp
                    ...
                    <td style="font-size:.78rem;color:#64748b;max-width:180px;">
                        <span style="display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:180px;" title="{{ $attempt->user_agent }}">
                            {{ $attempt->user_agent ? Str::limit($attempt->user_agent, 40) : '—' }}
                        </span>
                    </td>
                    <td class="pe-4" style="white-space:nowrap;">
                        <div style="font-size:.8rem;color:#0f172a;font-weight:500;">{{ $attempt->attempted_at->format('d M Y') }}</div>
                        <div style="font-size:.75rem;color:#94a3b8;">{{ $attempt->attempted_at->format('H:i:s') }}</div>
                    </td>
                </tr>
                @empty
                <tr><td colspan="6">
    ...
    <div style="padding:.85rem 1.25rem;border-top:1px solid #f1f5f9;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.5rem;">
        <span style="font-size:.78rem;color:#94a3b8;">Halaman {{ $attempts->currentPage() }} dari {{ $attempts->lastPage() }}</span>
        {{ $attempts->links() }}
    </div>
    @endif
</div>
